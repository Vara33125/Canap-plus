    ...
    public function publish(User $user, Request $request, EntityManagerInterface $em): Response
    {
        ...
        if (!in_array('ROLE_ADMIN', $roles) ) 
        {
            ...
            // Générer le message flash 
                $this->addFlash("success", "L'utilisateur {$user->getfirstName()} {$user->getLastName()} a été ajouté à la liste des administrateurs");
        ...
        elseif (in_array('ROLE_ADMIN', $roles) && !in_array('ROLE_SUPER_ADMIN', $roles) )
        {
            ...
            // Demander au manager des entités de préparer la requête de modification
                $em->persist($user);
            // Générer le message flash
                $this->addFlash("success", "L'utilisateur {$user->getfirstName()} {$user->getlastName()} a été retiré de la liste des administrateurs");
        }
